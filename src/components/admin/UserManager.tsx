    ...
    try {
      if (editingProfile) {
        ...
        if (!isAdmin || profile?.role !== 'admin') {
          toast({ title: 'Acesso não autorizado', description: 'Somente usuários admin podem criar usuários.', variant: 'destructive' });
          return;
        }
        const { data: { session }, error: sessionError } = await supabase.auth.getSession();
        if (sessionError || !session?.access_token) throw new Error('Sua sessão expirou. Faça login novamente.');
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button onClick={openDialog} disabled={!isAdmin || profile?.role !== 'admin'} title={!isAdmin || profile?.role !== 'admin' ? 'Somente usuários admin podem criar usuários.' : 'Criar usuário pelo backend seguro.'}>
              <Plus className="mr-2 h-4 w-4" />
              Novo Usuário (backend)
            </Button>
          </DialogTrigger>
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                {editingProfile 
                  ...
                  : 'A criação de usuários requer um backend administrativo seguro.'
                }
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    {roles.filter(role => editingProfile || role.value !== 'user').map(role => (
