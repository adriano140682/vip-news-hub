...
async function fetchRows(path) {
  ...
  const response = await fetch(`${supabaseUrl}/rest/v1/${path}`, { headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}` } });
...
try {
  ...
  const paths = siteUrl ? [
    ...
    ...articles.filter((article) => article.slug).map((article) => xmlUrl(`/artigo/${encodeURIComponent(article.slug)}`, article.published_at || article.updated_at)),
  ] : [];
  await mkdir('public', { recursive: true });
  await writeFile('public/sitemap.xml', `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${paths.join('')}</urlset>\n`);
  console.log(siteUrl ? `Sitemap gerado com ${paths.length} URLs.` : 'PUBLIC_SITE_URL não configurado; sitemap sem URLs absolutas até o domínio oficial ser informado.');
} catch (error) {
